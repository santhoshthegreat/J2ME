...
//		cal.set( Calendar.MONTH, Calendar.FEBRUARY);
		...
		for (int i = 0; i < abbreviations.length; i++)
		{
			...
			StringItem item = new StringItem( null, abbreviation, de.enough.polish.ui.StyleSheet.calendarweekdayStyle );
			set( i, 0, item );
		}
	...
	private void buildCalendar(Calendar cal)
	{
		...
		if (col > 0) {
			...
			for (int day = daysInMonth; --col >= 0; ) {
				...
				StringItem item = new StringItem( null, Integer.toString( day ), de.enough.polish.ui.StyleSheet.calendardayinactiveStyle );
		...
		for (int day = 1; day <= daysInMonth; day++) {
			if (isInCurrentMonth && day == this.lastDay) {
				...
				item = new StringItem( null, Integer.toString( day ), de.enough.polish.ui.StyleSheet.calendarcurrentdayStyle );				
			} else {
				//#style calendarDay?
				item = new StringItem( null, Integer.toString( day ), de.enough.polish.ui.StyleSheet.calendardayStyle );
			}
			item.setAppearanceMode( INTERACTIVE );
			set( col, row, item);
			col++;
...
//			System.out.println("day=" + day + ", col=" + col + ", row=" + row);
		...
		for (int day=1; col < 7; col++) {
			//#style calendarDayInactive?
			item = new StringItem( null, Integer.toString( day ), de.enough.polish.ui.StyleSheet.calendardayinactiveStyle );
			item.setAppearanceMode( INTERACTIVE );
			set( col, row, item);			
			day++;
		}
