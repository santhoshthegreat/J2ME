...
//# //			}
	...
	private void showPolishCalendar() {
		...
		//#style calendarForm?
		Form calForm = new Form( getLabel() , de.enough.polish.ui.StyleSheet.calendarformStyle );
		calForm.addCommand( StyleSheet.OK_CMD );
		calForm.addCommand( StyleSheet.CANCEL_CMD );
		calForm.setCommandListener( this );
		if (this.inputMode != TIME) {
			//#style calendar?
			CalendarItem item = new CalendarItem(cal, de.enough.polish.ui.StyleSheet.calendarStyle );
			this.calendarItem = item;
			calForm.append( item );
		}
		if (this.inputMode != DATE) {
			//#style time?
			TimeEntryItem time = new TimeEntryItem( null, cal.get(Calendar.HOUR_OF_DAY), cal.get(Calendar.MINUTE) , de.enough.polish.ui.StyleSheet.timeStyle );
			calForm.append( time );
			this.timeItem = time;
		}
